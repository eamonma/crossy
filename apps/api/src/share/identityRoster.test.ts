// Pins the API's identity palette to the cross-client contract (DESIGN.md §8,
// vectors/identity/roster.json). The server-rendered share card must wear the exact hexes the web,
// push, and iOS clients paint, so if the API's IDENTITY_ROSTER or its bucketing ever drifts from the
// frozen slots, this fails. The vector is the same language-neutral ground apps/web, apps/session,
// and iOS pin against, so a green run here means the card matches every client surface.
...
describe("API identity palette matches the cross-client vector (DESIGN.md §8)", () => {
...
describe("identityColor (resolve a wire color to the painted hex per ground)", () => {
  it("paints the slot's light variant on a light ground and the dark on a dark ground", () => {
    // #000000 buckets to slot 0 (the vector's first row).
    expect(identityColor("#000000", false)).toBe(IDENTITY_ROSTER[0]!.light);
    expect(identityColor("#000000", true)).toBe(IDENTITY_ROSTER[0]!.dark);
  });

  it("falls back to the given string when the wire color is malformed", () => {
    expect(identityColor("nonsense", false)).toBe("nonsense");
  });
});
