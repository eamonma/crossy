...
enum RoomFactsSheetLayout {
    ...
    /// The multiplayer check row is the 48 pt hold-to-propose control, NOT a 44 pt list row
    /// (the Wave 15.10 fix: counting it as 44 clipped the sheet by ~25 pt with the hint
    /// standing). The view pins the control and hint to these exact heights so the detent
    /// arithmetic stays honest (R7).
    ...
    static func height(
        ...
        if operationRows > 0 {
            let standardRows = CGFloat(operationRows - (holdCheckRow ? 1 : 0))
            rowBlock =
                operationsAirAbove + dividerHeight + operationsAirBelow
                + operationRowHeight * standardRows
                + (holdCheckRow ? holdControlHeight : 0)
                + (holdCheckRow && holdHint ? holdHintGap + holdHintHeight : 0)
        } else {
            rowBlock = 0
        }
                ...
                .presentationDetents([
                    .height(
                        RoomFactsSheetLayout.height(
                            ...
                            // honestly with its hint (R7; the Wave 15.10 clip fix).
