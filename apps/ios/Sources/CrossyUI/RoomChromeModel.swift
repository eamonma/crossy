...
public final class RoomChromeModel {
    ...
    // menus, so the chrome model carries no facts progress. The
    // inflate-from-the-pill morph retired with the PillInflation prototype.
    ...
    /// device's, not a screenshot script's. The facts sheet is a system
    /// presentation now too, so it lost its scripted entry with the morph.)
    ...
    /// tests ever see. Returns nil when the change was a cut. The melt is the
    /// only walk now, always on the critically damped chrome curve (the pill
    /// inflation's overshoot lever retired with the morph, 2026-07-12).
