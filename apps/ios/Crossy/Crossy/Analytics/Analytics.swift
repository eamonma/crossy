...
/// evidence rigs (-morphLab, -meltLab, -islandLab, -gooLab), the offline demo room (-demoRoom
/// and the -i2* scripts that imply it), the -i3Fixture walk, and the harness identity
/// (CROSSY_IT_TOKEN). These are all launch-time facts, so the selection is made once.
private func quietComposition() -> Bool {
    if ProcessInfo.processInfo.environment["XCODE_RUNNING_FOR_PREVIEWS"] == "1" {
        return true
    }
    let rigs = ["morphLab", "meltLab", "islandLab", "gooLab", "demoRoom", "i3Fixture"]
