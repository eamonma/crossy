"use strict";
(() => {
  // src/guardian/detect.ts
  var PUZZLE_PATH = /^\/crosswords\/[a-z][a-z-]*\/\d+$/;
  function isGuardianCrosswordPage(url) {
    let parsed;
    try {
      parsed = new URL(url);
    } catch {
  ...
  // src/guardian/extract.ts
  var CROSSWORD_ISLAND_SELECTOR = 'gu-island[name="CrosswordComponent"]';
  function parseCrosswordIslandProps(props) {
    if (props === null) {
    ...
    const document2 = parsed.data;
    if (typeof document2 === "undefined" || document2 === null) {
    ...
    return { ok: true, document: document2 };
  }
