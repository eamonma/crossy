...
final class RoomFactsSheetLayoutTests: XCTestCase {
    ...
    // when the grid is short, or the sheet clips its last row (R7: every conditional
    // counted honestly; the Wave 15.8 audit measured the clip at ~25 pt).
...
// The hold-to-propose fill under Reduce Motion (U3, Wave 15.8 fix): the old binary fill
// read as armed at touch-down and then "did nothing". The file's comment promises a
// STEPPED state: quarters advancing across the hold, full just before the commit.
