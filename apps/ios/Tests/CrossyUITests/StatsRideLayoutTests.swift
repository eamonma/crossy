...
// The stats morph's geometry (apps/ios/DESIGN.md §4: the time pill inflates
...
// pill's centered clock, at open exactly in the headline slot, and the card's
// height is slot arithmetic, so the hand-off on both ends is exact by
// construction.

final class StatsRideLayoutTests: XCTestCase {
    /// A stats-shaped morph: rest is the time pill's frame (the pill centers
    /// its clock, so the rest center IS the clock); open is the card hanging
    /// under the room bar, at the no-detail height.
    ...
    func test_rider_atRest_sitsExactlyOnTheTimePillsClock_ID2() {
