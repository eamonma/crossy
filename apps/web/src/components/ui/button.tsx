...
const buttonVariants = cva(
  // Control text sits below prose (text-0; lg, the gate CTAs, opts up to text-2) so buttons
  // read as controls, not sentences. Heights are rem literals, not h-N utilities: this
  // theme's Radix spacing steps make h-5..h-9 resolve to 24/32/40/48/64px, so a stock
  // shadcn h-7 quietly renders 40px. The targets are default 28, sm 24, xs 20, lg 36.
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-0 font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3",
  {
    variants: {
      variant: {
      ...
      size: {
        ...
        xs: "h-[1.25rem] gap-1 rounded-[min(var(--radius-md),10px)] px-1.5 in-data-[slot=button-group]:rounded-lg has-data-[icon=inline-end]:pr-1 has-data-[icon=inline-start]:pl-1",
        ...
        icon: "size-[1.75rem]",
        "icon-xs":
          "size-[1.25rem] rounded-[min(var(--radius-md),10px)] in-data-[slot=button-group]:rounded-lg",
        "icon-sm":
