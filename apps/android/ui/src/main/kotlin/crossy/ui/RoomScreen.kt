...
fun RoomScreen(
    ...
    Column(modifier = modifier.fillMaxSize().background(ground.tokens.canvas.toColor())) {
        ...
        if (frozen) {
            ...
            // host-ended). A host-ended room shows the one quiet abandoned notice where the deck was
            // (iOS abandonedZone); a completed room prints nothing (the deck just leaves and the time
            // pill's seal is the record, iOS `.completed`), a small spacer keeping the bottom breath.
            if (render.status == GameStatus.ABANDONED) {
