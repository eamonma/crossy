...
// it must never throw into the actor or socket path, so a vendor fault is one log line.
// Absent POSTHOG_TOKEN the noop runs and the SDK is never constructed, so dev machines and
// CI behave identically with zero analytics network.
...
/** One product event. The v1 vocabulary is snake_case (room_joined, solve_completed, ...). */
...
export interface Analytics {
  ...
  /** Flush the buffered queue and close the client. Hooked into the SIGTERM drain. */
 ...
 * present-but-empty value counts as absent (the 12-factor unset convention main.ts uses).
...
export function createPosthogAnalytics(
  ...
  return {
    ...
    async shutdown(): Promise<void> {
      ...
      // a flush fault on drain never aborts the rest of the shutdown sequence.
      try {
