...
The puzzle `title` and `author` are display metadata parsed from the uploaded document at ingestion. They are shown back verbatim and are never normalized or compared, so INV-1 casing does not apply, and they are not solutions, so INV-6 is untouched; ingestion entity-decodes, trims, and caps each at 200 characters, and absent, null, empty, or non-string all read as null. They live on the `puzzles` row (not on `ClientPuzzle`/`ServerPuzzle`, which the solve screen does not need), so they surface only where a client shows them back: `title` and `author` on `GET /puzzles`, `title` in the `puzzle` summary on `GET /games`, and both as the additive top-level `puzzleTitle`/`puzzleAuthor` on the `GET /games/{id}` view (the share card's masthead reads them there; the fields sit beside `puzzle`, which stays exactly `ClientPuzzle`).
