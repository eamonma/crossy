...
| `GET /me`                             | authenticated (self)                    | the caller's display identity `{userId, displayName, isAnonymous, avatarUrl, needsName}`; `displayName` is the app-DB value and MAY be null (the only place null crosses, so a client can detect a nameless account and onboard); `needsName` is the server-computed onboarding trigger `!isAnonymous && displayName === null` (the client holds no naming policy); works with zero games |
| `PATCH /me`                           | authenticated (self)                    | `{displayName}` sets the caller's display name; the server NFC-normalizes, trims, collapses internal whitespace, validates (1-40 graphemes, no control/zero-width/bidi-override, INV-1 casing does NOT apply), and returns the canonical `{userId, displayName, isAnonymous, avatarUrl, needsName}`; rejects `NAME_REQUIRED`/`NAME_TOO_LONG`/`NAME_INVALID` (422), a malformed body `VALIDATION` (400); rate-limited per user |
