...
| `GET /puzzles`                        | authenticated (own uploads)             | the caller's uploaded puzzles, newest first; per puzzle `{puzzleId, createdAt, rows, cols, features, title, author}` (`title`/`author` are display metadata, null when absent), no solution (INV-6); `limit` (default 50, max 100) + `createdAt`-cursor `before` pagination |
...
| `GET /games`                          | any authenticated user, guests included | the caller's games (membership join), newest first; per game `{gameId, name, role, createdAt, createdBy, memberCount, puzzle:{puzzleId, rows, cols, title}}` (`puzzle.title` is display metadata, null when absent), no board and no `status` (game_state is session-owned, DESIGN.md section 9); `limit` (default 50, max 100) + `createdAt`-cursor `before` pagination |
| `POST /games/{id}/join`               | any authenticated user, guests included | `{code}` creates membership with role `spectator`                                                                              |
| `POST /games/join`                    | any authenticated user, guests included | `{code}` resolves the game by invite code alone (no `gameId`) and joins as `spectator`, same semantics as `/{id}/join`; returns `{gameId, role, userId}` (the resolved `gameId` lets a code-only caller open the view and WS); a code matching no game is `GAME_NOT_FOUND` |
...
The two list endpoints (`GET /games`, `GET /puzzles`) back the signed-in home. Both are cursor-paginated, never offset: a `limit` clamped to `[1, 100]` (default 50) and an optional `before`, an ISO 8601 `createdAt` the page filters strictly before (`created_at < before`); the client pages by passing the last row's `createdAt` as the next `before`. A present but unparseable `before` is `VALIDATION` (400). Both carry only INV-6-safe puzzle geometry (rows, cols) projected server-side, never the solution-bearing snapshot. Visibility is scoped to the caller: `GET /games` returns only games the caller is a member of (any role, guests included), and `GET /puzzles` returns only puzzles the caller uploaded. `GET /games` deliberately omits a lifecycle `status`: a game's status (ongoing, completed, abandoned) lives in the session-owned `game_state`, which the core API holds no read grant on (DESIGN.md section 9), so it cannot report completion from its own tables; when the Archive module's planned read grant lands, a `status` field is an additive extension.
