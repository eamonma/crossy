...
| `GET /games/{id}/attribution`         | member, completed game only             | the post-game first-correct owner map: `{owners: {[cell]: userId}}`, who FIRST placed the correct value in each cell (the Archive read model, computed on read from `cell_events`, DESIGN.md section 7, section 9). userIds only, no solution content (INV-6); the client already holds the roster from the game view. Gated to completed games (an ongoing game's owner map leaks solving progress; an abandoned-game recap is deferred): a game that is not completed is `GAME_NOT_FOUND`, and a non-member is `NOT_PARTICIPANT`, the same gate as the game view |
