...
| `GET /puzzles`                        | authenticated (own uploads)             | the caller's uploaded puzzles, newest first; per puzzle `{puzzleId, createdAt, rows, cols, features}`, no solution (INV-6); `limit` (default 50, max 100) + `createdAt`-cursor `before` pagination |
