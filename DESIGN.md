...
- Extension auth is the same bearer as the SPA; how the extension obtains the signed-in user's token (the pairing handshake with the web app; `externally_connectable` messaging is the leading proposal) is the extension track's to pin (section 15). Layering is unchanged: `apps/extension` imports `packages/protocol` only, enforced like every other app.
...
- **Scope is per-account, not global.** The digest is unique **per `created_by`**, so a match means "you already added this one," never "someone, somewhere, already has this." Global dedup would leak a cross-account fact ("this puzzle exists in Crossy") that muddies the extraction-only posture (D21): each user's uploads stay private to them, and dedup must not become a back channel that reveals another account's library. Per-account also sidesteps the starter seed cleanly: every new full account is seeded its own copy of the one starter puzzle (#118), so a global digest would collide every new account against the first, while a per-account digest sees each seed as that account's own first row. The seed writes `puzzles` directly, not through `POST /puzzles`, so it never runs the dedup path, but the per-account scope means it would be correct even if it did.
- **Behavior never blocks, and each surface reads it differently.** The wire is one shape: a fresh insert is `201` with the new `PuzzleView`; a duplicate is `200` with the caller's existing `PuzzleView` plus `duplicate: true`. Never a `4xx`, since the point is that a re-import of a puzzle you want is never rejected, only resolved to the copy you already hold. The three surfaces present it to fit their moment:
  - **The extension** is why dedup exists: it re-posts today's puzzle on every visit, ignores the marker, and treats `200` exactly like `201` (it already keys success on `puzzleId`, `apps/extension/src/api.ts`), so a daily re-post lands silently on the existing row with no clutter and no UI.
  - **The web upload** is a deliberate act, so it reads the marker and shows a quiet "You already have this one" that opens the existing puzzle. Because the match is whole-object strict, that message is almost never wrong, so it needs no escape hatch to be trustworthy; whether to still offer an explicit "add a copy anyway" is a decision point below (the draft says no, a whole-object match is a byte-identical copy and a second is pure noise).
  - **iOS** has no create or upload surface yet, so dedup is not surfaced there; when an upload path lands it inherits the same `200`/`duplicate` contract.
- **Storage is a digest column plus a partial unique index**, `puzzles.content_digest` with `UNIQUE (created_by, content_digest)`, added expand/contract under the single writer (API owns `puzzles`, INV-7). The column is nullable and backfilled lazily: existing rows read as un-digested and never collide, and the unique index is partial (`WHERE content_digest IS NOT NULL`) so it constrains only new digested rows. The insert becomes an upsert, `ON CONFLICT (created_by, content_digest) DO NOTHING` then re-select the existing row, which makes the dedup atomic under the concurrent-same-puzzle race two tabs or two extension clicks create, without an application-level read-then-write gap. A lookup table was rejected: a column on the row it identifies is simpler, and the unique index is the dedup mechanism itself, not a secondary structure to keep in sync.

The decision points the owner settles: whether identity is the whole object (the draft, fewest false positives) or a narrower solution-only key (fewer false negatives, the prior draft); whether the web upload offers an explicit "add a copy anyway" beside the existing-copy affordance (the draft says no); whether scope is per-account or global-with-a-private-response (the draft says per-account); and whether existing un-digested rows are backfilled and collapsed or left alone (the draft leaves them, dedup is forward-only). These are recorded in D23 and in the vector README, not resolved here.
...
**D23 Ingest dedup on a per-account whole-puzzle digest; idempotent 200 with a `duplicate` marker, never a block (pending owner ratification).** The extension makes re-ingesting the same daily puzzle the common case, so `POST /puzzles` collapses a re-post to the row the caller already holds instead of growing a copy. The governing principle (owner ruling 2026-07-12) is that a false positive costs far more than a false negative: a duplicate copy is harmless clutter, but rejecting or mislabeling a puzzle a user deliberately imports is real friction, so the match is strict and the ingest never blocks. Identity is a digest over the whole translated `ServerPuzzle` in canonical form: `rows`, `cols`, sorted `blocks`, sorted `circles`/`shadedCircles`, the ASCII-uppercased `solution`, and the clue list in grid-derived order carrying each clue's normalized plain `text` (the projection, stable across the clue-runs wave, so `runs` never enters the hash). Every field a solver sees is in the hash, so a match is a near-certain true duplicate; the cost is false negatives (the same puzzle from two outlets may hash apart), which the principle accepts. The canon is pinned by `vectors/puzzle-digest/` before the code. Scope is per-`created_by`, so a hit means "you already have this," not a cross-account leak, which also keeps the per-account starter seed (#118) collision-free. Behavior is one wire shape for both clients but read differently per surface: a `200` returns the existing `PuzzleView` plus `duplicate: true`; the extension ignores the marker and dedups silently, the web upload reads it and shows a quiet "you already have this one" that opens the existing puzzle, and iOS has no upload surface yet so it is not surfaced. Storage is `puzzles.content_digest` with a partial `UNIQUE (created_by, content_digest)`, expand/contract under the API's single-writer, and an `ON CONFLICT DO NOTHING` upsert that is atomic under the two-clicks race. The digest takes the `solution` as an input, so it is a solution oracle and stays server-only, off every wire type (INV-6). Rejected: a `4xx`/`409` block (the friction the principle exists to avoid, and it forces both clients to special-case a non-error); a solution-only or geometry-only digest (looser, so two puzzles sharing a grid could collapse, a false positive the principle refuses); global scope (leaks that a puzzle exists across accounts, wrong for D21's extraction-only posture); a vendor-id/date key (only some formats carry one, not uniform, and invites spoofing a collision). Cost: one server-side digest function held to a vector suite, one expand/contract migration, an added `200` branch, and accepted false negatives (a re-import from a different outlet stores a second copy). **Adopted-by-default once ratified:** the sub-choices (whole-object vs solution-only identity, an explicit web "add a copy anyway", per-account vs global-with-private-response, backfill-and-collapse vs forward-only) are recorded in section 15.
...
- Extension extraction points unverified (Phase 6): where each outlet's document actually lives in its page (the NYT v6 JSON in page state, the Guardian embedded JSON, the AmuseLabs encoded blob and its variants) is coded from references (xword-dl, MIT, as a parsing reference), not yet confirmed against live pages. Confirm per adapter at implementation, with synthetic fixtures pinned first. The extension-to-web pairing handshake (how the extension obtains the user's bearer, section 7) is also open; pin it in Wave 6.2.
- Ingest dedup sub-choices (D23, pending ratification): the calls the owner settles when ratifying D23, all downstream of the governing principle that a false positive costs more than a false negative. (1) Identity breadth: the draft hashes the whole puzzle object (geometry, circles, solution, and normalized clue text), the strictest key and so the fewest false positives; a narrower solution-only key (the prior draft) collapses more near-duplicates but risks merging two different puzzles that share a grid. (2) Web "add a copy anyway": the draft does not offer one, since a whole-object match is a byte-identical copy and a second is pure noise; add it only if a real case wants two rows of one puzzle. (3) Scope: per-account is recommended over global; a middle option, global dedup with a per-account response that never reveals another account holds the puzzle, is possible but adds a global index and a leak surface for no product gain at friends scale. (4) Existing duplicates: the draft is forward-only, un-digested rows predating the column are left alone; a one-time backfill that digests and dedups history is a separate, optional migration. Close these at D23 ratification; until then implementations MUST NOT ship a digest that disagrees with `vectors/puzzle-digest/`.
