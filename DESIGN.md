...
**Duplicate detection on ingest (D23, pending owner ratification).** The extension makes duplicates the common case, not the exception: the same daily NYT puzzle is ingested again and again, by one account across days and by several accounts on the same day. A registry that silently grows a new row per re-post is wrong twice over, it clutters a user's own library with copies of one puzzle, and it wastes the one moment where the system knows two documents are the same. `POST /puzzles` therefore dedups on a **per-account content digest**, and the recommended shape is:

- **Identity is a solution-derived digest**, computed at the ACL boundary once the document has translated to a `ServerPuzzle` and passed every named domain check. The digest is over the stable, solution-bearing identity of the puzzle: `rows`, `cols`, the sorted `blocks` set, and the per-cell `solution` array, ASCII-uppercased (INV-1), in row-major order. It is **not** over clue text: entity and markup variance across outlets, and the landing clue-runs wave (which reprojects `text` from structured `runs`), would split two ingests of one puzzle. Numbering is grid-derived, never trusted from the document (`nyt.ts`, `guardian.ts`), so the geometry already carries every identity the clues would. Circles and title/author are excluded for the same reason, a circle overlay or a byline is not the solve. The canon and its digest are pinned by `vectors/puzzle-digest/` **before** the implementation, the house rule; that suite is the golden the API's digest function must match, in the one place the digest is computed.
- **The digest never crosses the wire (INV-6).** It is derived from the solution, so two documents hashing equal reveals they are the same puzzle: the digest is itself a solution oracle. It lives only in a server-side column and never appears on any `ClientPuzzle`, any list row, or the `POST /puzzles` response. This is the same discipline as the solution itself, enforced by keeping the digest off every wire type, not by runtime stripping.
...
- **Behavior is idempotent, a `200` returning the existing puzzle's view plus a `duplicate` marker,** not a `4xx`. The extension wants silent idempotency: a re-post of today's puzzle should quietly resolve to the row the user already has, not surface an error the user must dismiss. The web upload surface wants the opposite affordance, a visible "you already have this one" so the user is not confused by a no-op. One response serves both: the existing `PuzzleView` (so a caller always leaves with a usable `puzzleId`) plus `duplicate: true`, at `200` rather than the `201` a fresh insert returns. The extension ignores the marker and treats `200` exactly like `201` (it already keys success on the `puzzleId`, `apps/extension/src/api.ts`); the web surface reads the marker and shows the affordance. A `409` was rejected: it forces both clients to special-case a status that is not really an error, and the extension would have to parse the conflict body for the existing id anyway. See PROTOCOL.md section 12 for the wire shape.
...
The recommended option is deliberately narrow. Three decision points are the owner's to settle: whether the digest excludes circles (the draft says yes, a circle variant is the same solve), whether scope is per-account or global-with-a-private-response, and whether an existing un-digested duplicate is backfilled and collapsed or left alone (the draft leaves it alone, dedup is forward-only). These are recorded in D23 and in the vector README, not resolved here.
...
**D23 Ingest dedup on a per-account, solution-derived content digest; idempotent 200 with a `duplicate` marker (pending owner ratification).** The extension makes re-ingesting the same daily puzzle the common case, so `POST /puzzles` collapses a re-post to the row the caller already holds instead of growing a copy. Identity is a digest over `rows`, `cols`, sorted `blocks`, and the ASCII-uppercased `solution` (section 7), never clue prose (outlet and clue-runs variance would split one puzzle) and never circles or metadata; the canon is pinned by `vectors/puzzle-digest/` before the code. Scope is per-`created_by`, so a hit means "you already have this," not a cross-account leak, which also keeps the per-account starter seed (#118) collision-free. Behavior is a `200` returning the existing `PuzzleView` plus `duplicate: true`, so the extension gets silent idempotency and the web surface gets a visible affordance from one response. Storage is `puzzles.content_digest` with a partial `UNIQUE (created_by, content_digest)`, expand/contract under the API's single-writer, and an `ON CONFLICT DO NOTHING` upsert that is atomic under the two-clicks race. The digest is solution-derived, so it is a solution oracle and stays server-only, off every wire type (INV-6). Rejected: global scope (leaks that a puzzle exists across accounts, wrong for D21's extraction-only posture); a bare `409` (forces both clients to special-case a non-error and re-parse the body for the existing id); a geometry-only digest (two genuinely different puzzles can share a block pattern, so it over-collapses); a clue-inclusive digest (splits identical daily puzzles across outlets and across the clue-runs change); a vendor-id/date key (only some formats carry one, so it is not a uniform identity and invites spoofing a collision). Cost: one server-side digest function held to a vector suite, one expand/contract migration, and an added `200` branch on a path that only ever returned `201`. **Adopted-by-default once ratified:** the three open sub-choices (circle exclusion, per-account vs global-with-private-response, backfill-and-collapse vs forward-only) are cheap to revisit and are recorded in section 15.
...
- Ingest dedup sub-choices (D23, pending ratification): three cheap-to-revisit calls the owner settles when ratifying D23. (1) Circle exclusion: the draft digests geometry plus solution and excludes circles, so a circled and an uncircled ingest of one puzzle collapse to one row; revisit only if a real corpus shows a circle overlay that should count as a distinct artifact. (2) Scope: per-account is recommended over global; a middle option, global dedup with a per-account response that never reveals another account holds the puzzle, is possible but adds a global index and a leak surface for no product gain at friends scale. (3) Existing duplicates: the draft is forward-only, un-digested rows predating the column are left alone, not backfilled and collapsed; a one-time backfill that digests and dedups history is a separate, optional migration. Close all three at D23 ratification; until then implementations MUST NOT ship a digest that disagrees with `vectors/puzzle-digest/`.
