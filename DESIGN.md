...
**Games module**: create (full accounts only), join by invite code (any authenticated user, guests included; role starts `spectator`), self-upgrade to `solver`, kick (denylist plus notify; the host cannot kick themselves), abandon (host only), and the game view (solution-stripped puzzle, membership, session endpoint; no board, since the API holds no `game_state` read grant and the mutable board arrives over the WebSocket, PROTOCOL.md section 12). Host succession: if the host is tombstoned or deletes their account, host passes to the earliest-joined remaining `solver`; if none remains, the game auto-abandons, so a game is never left unadministrable.
