// The Supabase adapter (SP2). It verifies access tokens against an in-memory JWKS that a
...
export interface SupabaseAuthPort extends AuthPort {
...
/**
 * Construct the Supabase auth port. Performs the boot JWKS fetch (SP2 step 1), so a
 ...
 */
export async function createSupabaseAuthPort(
  config: SupabaseAuthConfig,
): Promise<SupabaseAuthPort> {
